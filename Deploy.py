...
class VideoSavor:
	...
	def __init__(self, targetFileName, videoCapture):
		...
		self.outputStream = cv2.VideoWriter(targetFileName + ".mp4", fourcc, frameRate, (width, height) )
...
def DetectViolence(PATH_FILE_NAME_OF_SOURCE_VIDEO, PATH_FILE_NAME_TO_SAVE_RESULT):
	...
	if shouldSaveResult:
		videoSavor = VideoSavor(PATH_FILE_NAME_TO_SAVE_RESULT, videoReader)
...
if __name__ == '__main__':
	if len(sys.argv) >= 2:
		...
		print(os.path.isfile(PATH_FILE_NAME_OF_SOURCE_VIDEO))
		if os.path.isfile(PATH_FILE_NAME_OF_SOURCE_VIDEO):
			...
			raise ValueError("Not such file")
	else:
		printHelp()
...
def Solution(source,destination):
	...
	PATH_FILE_NAME_OF_SOURCE_VIDEO = source

	try:
		PATH_FILE_NAME_TO_SAVE_RESULT = destination
	except:
		PATH_FILE_NAME_TO_SAVE_RESULT = None
	print(os.path.isfile(PATH_FILE_NAME_OF_SOURCE_VIDEO))
	if os.path.isfile(PATH_FILE_NAME_OF_SOURCE_VIDEO):
